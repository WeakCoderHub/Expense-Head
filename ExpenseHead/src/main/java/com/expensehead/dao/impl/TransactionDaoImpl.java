...
public class TransactionDaoImpl implements TransactionDao {
	...
	public int saveExpense(Transactions transaction, User user) {
		...
		{
			Query q1 = session.createQuery("update User set payable = payable - '"+userPayable+"',receivable = receivable +'"+amountDifference+"' where userId = '"+userId+"'");
			q1.executeUpdate();
	...
	public int saveContribution(Journal journal, User user) {
		...
		if (creditAmount <= userPayable) {
			...
			Query q2 = session
					.createQuery("update User set payable = payable - :amount where userId = :userId");
		...
		{
			Query q1 = session.createQuery("update Group set depositsLeft = depositsLeft + :creditAmount, payback = payback + :amountDifference where groupId = :groupId");
	...
	public int settleDues(Journal journal, User user) {
		...
		if (debitAmount <= userReceivable) {
			Query q1 = session.createQuery("update Group set depositsLeft = depositsLeft - :amount, payback = payback - :amount where groupId = :groupId");
			q1.setParameter("amount", debitAmount);
			q1.setParameter("groupId", journal.getGroupId());
			result = q1.executeUpdate();

			Query q2 = session
					.createQuery("update User set receivable = receivable - :amount where userId = :userId");
		...
		{
			Query q1 = session.createQuery("update Group set depositsLeft = depositsLeft - :debitAmount, payback = payback - :receivable where groupId = :groupId");
	...
	public List<Journal> getJournalDetails() {
		...
		Query q = session.createQuery("from Journal");
